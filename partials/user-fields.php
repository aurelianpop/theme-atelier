...
<table class="form-table">
    <?php if ((is_object($user) && in_array('parent', $user->roles)) || (is_object($user) && in_array('administrator', $user->roles)) || !is_object($user)) {
        ...
        <tr class="ta-table-help">
            ...
            <td>
                ...
                <select name="help">
                    <option <?php echo $help == 'financial' ? 'selected="selected"' : ''; ?> value="financial">
                        Financial
                    </option>
                    <option <?php echo $help == 'clothing' ? 'selected="selected"' : ''; ?> value="clothing">
                        Clothing
                    </option>
                    <option <?php echo $help == 'requisites' ? 'selected="selected"' : ''; ?> value="requisites">
                        Requisites
                    </option>
                    <option <?php echo $help == 'furniture' ? 'selected="selected"' : ''; ?> value="furniture">
                        Furniture
                    </option>
        ...
        <tr class="ta-table-added-children"
            style="<?php echo is_object($user) && get_the_author_meta('children', $user->ID) ? 'display:table-row' : 'display: none'; ?>">
            <?php is_object($user) ? $children = get_the_author_meta('children', $user->ID) : $children = array(); ?>
            <th><label for="added-children">Added Children</label></th>
            <td class="ta-children-list">
                <?php foreach ($children as $child_id) {
                    $child = get_post($child_id); ?>
                    <li><?php echo $child->post_title ?> <a href="javascript:void(0)" class="remove-child">[x]</a><input
                            type="hidden" name="children[]" value="<?php echo $child->ID; ?>"></li>
                <?php } ?>
            </td>
        </tr>
        <tr class="ta-table-anonymus">
            ...
            <td>
                ...
                <input type="radio" <?php echo $anonymus == '0' ? 'checked="checked"' : ''; ?> name="anonymus"
                       value="0"> Yes
                <input type="radio" <?php echo $anonymus == '1' ? 'checked="checked"' : ''; ?> name="anonymus"
                       value="1"> No<br>
        ...
        <tr class="ta-table-status">
            ...
            <td>
                ...
                <input type="radio" <?php echo $married == '0' ? 'checked="checked"' : ''; ?> name="married" value="0">
                Not Married
                <input type="radio" <?php echo $married == '1' ? 'checked="checked"' : ''; ?> name="married" value="1">
                Married<br>
    ...
    if ((is_object($user) && in_array('partner', $user->roles)) || (is_object($user) && in_array('administrator', $user->roles)) || !is_object($user)) {
        ...
        <tr id="pt-logo-cont" class="ta-table-logo">
            ...
            <td>
                ...
                <p class="hide-if-no-js" data-metaboxid="pt-logo-cont">
                    <a class="upload-custom-img <?php if ($src) {
                        echo 'hidden';
                    } ?>"
                       href="">
                        <?php _e('Set Partner Logo') ?>
                    </a>
                    <a class="delete-custom-img <?php if (!$src) {
                        echo 'hidden';
                    } ?>"
                       href="#">
        ...
        <tr class="ta-table-contact-name">
            ...
            <td>
                <input type="text" name="contact_name" id="contact-name"
                       value="<?php echo is_object($user) ? esc_attr(get_the_author_meta('contact_name', $user->ID)) : ''; ?>"
                       class="regular-text"/><br/>
        ...
        <tr class="ta-table-contact-surname">
            ...
            <td>
                <input type="text" name="contact_surname" id="contact-surname"
                       value="<?php echo is_object($user) ? esc_attr(get_the_author_meta('contact_surname', $user->ID)) : ''; ?>"
                       class="regular-text"/><br/>
        ...
        <tr class="ta-table-contact-phone">
            ...
            <td>
                <input type="text" name="contact_phone" id="contact-phone"
                       value="<?php echo is_object($user) ? esc_attr(get_the_author_meta('contact_phone', $user->ID)) : ''; ?>"
                       class="regular-text"/><br/>
        ...
        <tr class="ta-table-contact-function">
            ...
            <td>
                <input type="text" name="contact_function" id="contact-function"
                       value="<?php echo is_object($user) ? esc_attr(get_the_author_meta('contact_function', $user->ID)) : ''; ?>"
                       class="regular-text"/><br/>
